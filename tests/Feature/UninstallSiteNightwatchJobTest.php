...
test('handle creates task and runs uninstall script', function () {
    ...
    $job = new UninstallSiteNightwatch($this->site->id);
    $job->handle();
...
test('handle stops nightwatch before removing config', function () {
    Process::fake([
        '*' => Process::result(exitCode: 0),
    ]);
    ...
    expect($task->script)
        ->toContain('supervisorctl stop site-'.$this->site->id.'-nightwatch:*')
        ->toContain('rm -f /etc/supervisor/conf.d/site-'.$this->site->id.'-nightwatch.conf');
});
...
test('handle sets nightwatch_enabled to false', function () {
...
test('handle returns early when user has no ssh key', function () {
    ...
    $job = new UninstallSiteNightwatch($site->id);
    $job->handle();
