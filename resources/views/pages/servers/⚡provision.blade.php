...
new class extends Component
{
    ...
    public function mount(Server $server): void
    {
        ...
        $this->provisionUrl = $server->provision_token ? url('/provision/'.$server->provision_token) : '';
    }
...
<div>
    ...
    @if($this->server->provisioned_at)
        <flux:callout icon="check-circle" color="green">
            <flux:callout.heading>Server Provisioned</flux:callout.heading>
            <flux:callout.text>
                This server was marked as provisioned on {{ $this->server->provisioned_at->format('M j, Y \a\t g:i A') }}.
            </flux:callout.text>
        </flux:callout>
    ...
    @else
        ...
        <div class="mt-6 flex gap-2">
            <flux:button wire:click="markAsProvisioned" wire:confirm="Mark this server as provisioned? This will invalidate the provisioning URL.">
                Mark as Provisioned
            </flux:button>
